...
public class AgentRunner {
    ...
    public AgentRunResult run(AgentRunSpec spec) {
        ...
        try {
            ...
            for (int iteration = firstIteration; iteration <= spec.getMaxIterations(); iteration++) {
                ...
                List<AgentToolCall> calls = parser.parseToolCalls(response);
                ...
                if (finish != null) {
                    ...
                    conversation.appendToolMessages(context.getMessages(), response, calls, results);
    ...
    private AiToolCallResult requestModel(AgentRunSpec spec, AgentRunContext context) {
        if (spec.getModelRole() == AgentRunSpec.ModelRole.UTILITY) {
            return aiClient.chatWithMessagesUtility(context.getMessages(), toolExecutor.toolSchemas(context.isVerificationEnabled()));
        }
        return aiClient.chatWithMessagesRequired(context.getMessages(), toolExecutor.toolSchemas(context.isVerificationEnabled()));
    }
