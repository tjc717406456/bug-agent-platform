...
create table if not exists project (
  ...
  environments varchar(512) not null default 'prod,test' comment '项目环境列表，逗号分隔',
  schema_consistent tinyint not null default 1 comment '各环境表结构是否一致',
  schema_reference_env varchar(32) not null default 'test' comment '结构参考环境',
  created_at datetime not null comment '创建时间',
  updated_at datetime not null comment '更新时间',
...
create table if not exists analysis_record (
  ...
  environment varchar(32) comment '问题发生环境',
  database_access_level varchar(32) comment '实际数据库权限:NONE/SCHEMA_ONLY/BUSINESS_DATA',
  schema_datasource_id bigint comment '结构核对数据源ID',
  business_datasource_id bigint comment '业务数据核对数据源ID',
